...
describe('Logger', () => {
  ...
  test('error method should log correct message to wlog.error', () => {
    const obj = {
      message,
      customString
    }
    ...
    expect(wlog.error).toHaveBeenCalledWith(customString)
  })

  test('error method should format message when customString is not provided', () => {
    const obj = {
      message
    }
    ...
    expect(wlog.error).toHaveBeenCalledWith(formattedMessage)
  })

  test('error method with full parameters', () => {
    const obj: IMessage = {
      message,
      clientIP: '1.2.3.4',
      method: 'POST',
